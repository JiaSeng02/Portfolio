...
Mock.mock("/api/blogtype", "get", {
  code: 0,
  msg: "",
  // 生成一个名为 data 的数组, 数组的长度在 10 到 20 之间
  "data|10-20": [
    {
      "id|+1": 1, //这个属性表示 id 的值从 1 开始，每生成一个对象，id 的值会自动加
      name: "分类@id", // 这个属性表示 name 的值是一个字符串，其中 @id 会被替换为当前对象的 id 值。例如，如果 id 是 1，那么 name 的值就是 "分类1"。
      "articleCount|0-300": 0, // 表示 articleCount 的值是一个随机数，范围在 0 到 300 之间. 该分类下文章的数量.
      "order|+1": 1, //这个属性表示 order 的值从 1 开始，每生成一个对象，order 的值会自动加 1
    },
  ],
});
...
Mock.mock(/^\/api\/blog(\?.+)?$/, "get", function (options) {
  ...
  return Mock.mock({
    ...
    msg: "",
    data: {
      "total|2000-3000": 0, //随机数, 2000 到 3000 之间
      [`rows|${query.limit || 10}`]: [
        //默认为 10
        {
          id: "@guid",
          ...
          //生成一个 300x250 像素的随机图片, 背景色为黑色，文字颜色为白色，图片的描述为“Random Image”
          createDate: `@date('T')`,
        },
      ],
    },
